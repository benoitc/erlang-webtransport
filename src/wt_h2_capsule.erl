...
roundtrip_test_() ->
    Capsules = [
        padding(10),
        wt_stream(1, <<"hello">>),
        wt_stream_fin(2, <<"world">>),
        reset_stream(3, 0),
        stop_sending(4, 1),
        max_data(1000000),
        max_stream_data(5, 500000),
        max_streams_bidi(100),
        max_streams_uni(100),
        data_blocked(999999),
        stream_data_blocked(6, 888888),
        streams_blocked_bidi(50),
        streams_blocked_uni(25),
        close_session(0, <<>>),
        close_session(1, <<"error reason">>),
        drain_session(),
        datagram(<<"dgram data">>)
    ],
    [?_assertEqual({ok, C, <<>>}, decode(encode(C))) || C <- Capsules].

wt_stream_helper_test() ->
    ?assertEqual({wt_stream, 1, <<"data">>}, wt_stream(1, <<"data">>, false)),
    ?assertEqual({wt_stream_fin, 1, <<"data">>}, wt_stream(1, <<"data">>, true)).

decode_all_test() ->
    C1 = encode(wt_stream(1, <<"one">>)),
    C2 = encode(max_data(1000)),
    C3 = encode(datagram(<<"dg">>)),
...
type_name_test_() ->
    [
        ?_assertEqual(padding, type_name(?WT_PADDING)),
        ?_assertEqual(wt_stream, type_name(?WT_STREAM)),
        ?_assertEqual(close_session, type_name(?WT_CLOSE_SESSION)),
        ?_assertEqual(datagram, type_name(?DATAGRAM)),
        ?_assertEqual(999, type_name(999))
    ].
