...
new_stream_test() ->
    S = new(4, bidi, 1000),
    ?assertEqual(4, id(S)),
    ?assertEqual(bidi, type(S)),
    ?assertEqual(open, state(S)),
    ?assertEqual(1000, send_window(S)),
    ?assertEqual(1000, recv_window(S)),
    ?assert(is_open(S)),
...
send_within_window_test() ->
    S = new(4, bidi, 100),
    {ok, Data, S1} = send(S, <<"hello">>),
    ?assertEqual(<<"hello">>, Data),
    ?assertEqual(<<>>, send_buffer(S1)).

send_exceeds_window_test() ->
    S = new(4, bidi, 5),
    {ok, Data, S1} = send(S, <<"hello world">>),
    ?assertEqual(<<"hello">>, Data),
    ?assertEqual(<<" world">>, send_buffer(S1)).

close_local_test() ->
    S = new(4, bidi, 100),
    {ok, S1} = close_local(S),
    ?assertEqual(half_closed_local, state(S1)),
    ?assertNot(is_writable(S1)),
    ?assert(is_readable(S1)).

close_remote_test() ->
    S = new(4, bidi, 100),
    {ok, S1} = close_remote(S),
    ?assertEqual(half_closed_remote, state(S1)),
    ?assert(is_writable(S1)),
    ?assertNot(is_readable(S1)).

full_close_test() ->
    S = new(4, bidi, 100),
    {ok, S1} = close_local(S),
    {ok, S2} = close_remote(S1),
    ?assertEqual(closed, state(S2)),
    ?assertNot(is_writable(S2)),
    ?assertNot(is_readable(S2)).

stream_type_test_() ->
    [
        ?_assertEqual(bidi, stream_type(0)),
        ?_assertEqual(bidi, stream_type(1)),
        ?_assertEqual(uni, stream_type(2)),
        ?_assertEqual(uni, stream_type(3)),
        ?_assertEqual(bidi, stream_type(4)),
        ?_assertEqual(bidi, stream_type(5)),
        ?_assertEqual(uni, stream_type(6)),
        ?_assertEqual(uni, stream_type(7))
    ].

initiator_test_() ->
    [
        ?_assertEqual(client, initiator(0)),
        ?_assertEqual(server, initiator(1)),
        ?_assertEqual(client, initiator(2)),
        ?_assertEqual(server, initiator(3)),
        ?_assertEqual(client, initiator(4)),
        ?_assertEqual(server, initiator(5))
    ].
